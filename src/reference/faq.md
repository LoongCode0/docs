...
可以（v0.8.5 起，仅 Claude 任务）。LoongCode 里的 Claude 会话与 Claude CLI 共用同一份会话记录：在**同一个项目目录**下打开系统终端、运行 `claude` 并使用 `/resume`，即可在会话列表中看到 LoongCode 里的历史会话并继续对话。Codex 与 OpenCode 任务不适用。
