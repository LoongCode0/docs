...
LoongCode 应用本身可从 [官网](https://loongcode.cc/zh/#download) 免费下载，源码仓库公开发布。

使用 AI 功能需要本机安装 Claude CLI、Codex CLI、OpenCode CLI 或 Kimi Code CLI 中的任意一个，AI 服务的费用（如 API 调用计费、订阅计划等）由各 CLI 对应的供应商决定，与 LoongCode 本身无关，请参阅你所使用的 CLI 文档了解具体计费规则。
...
### Claude、Codex、OpenCode 和 Kimi 任务可以同时用吗？

可以。每个任务在创建时可独立选择使用 Claude CLI、Codex CLI、OpenCode CLI 或 Kimi Code CLI，彼此互不影响，可同时运行多个不同 CLI 的任务。多 CLI 能力自 v0.7.0 起支持，OpenCode 家族自 v0.9.0 起加入，Kimi 家族自 v0.10.0 起加入。
...
### 能在终端里继续 LoongCode 的会话吗？

可以（v0.8.5 起，仅 Claude 任务）。LoongCode 里的 Claude 会话与 Claude CLI 共用同一份会话记录：在**同一个项目目录**下打开系统终端、运行 `claude` 并使用 `/resume`，即可在会话列表中看到 LoongCode 里的历史会话并继续对话。Codex、OpenCode 与 Kimi 任务不适用。
...
**macOS** 请手动从 [LoongCode 官网](https://loongcode.cc/zh/#download) 下载最新的 `.dmg` 文件覆盖安装；应用内自动更新的平台支持范围以官方发布说明为准。
...
v0.9.4 起，应用图标已全套换新为「衔尾圆环龙」徽章（玄墨底金色圆环龙）。若升级后任务栏或桌面快捷方式仍显示旧图标，一般是操作系统的图标缓存尚未刷新——Windows 上可注销并重新登录（或重启「Windows 资源管理器」进程），仍不生效时重启电脑即可；应用窗口内左上角的 Logo 不受缓存影响，升级后立即更新。
...
- **主题：** 在"主题"处点选「深色 · 玄墨 / 浅色 · 宣纸 / 护眼 · 竹青」三套主题卡片之一，切换即时生效；也可打开「跟随系统」开关，随系统深浅色自动切换。
