...
MCP（Model Context Protocol，模型上下文协议）是让 AI 连接外部工具和数据源的开放协议。通过添加 MCP 服务器，Claude 或 Codex 可以在对话中直接调用数据库查询、文件系统、Web 搜索等能力。
...
- 启用开关 — 即时启停；Claude 模式下若未关联工作区任务，**工作区**作用域的服务器开关不可操作（Codex 模式无此限制）
...
在列表行右侧拨动开关即可即时启用或停用某个服务器，无需重新填写配置。

> Claude 模式下，若未关联工作区任务，**工作区**作用域的服务器开关不可操作。可先在任务列表中点击一个工作区任务再进行切换。
