...
> 若尚未安装 Claude CLI、Codex CLI、OpenCode CLI 或 Kimi Code CLI，请先参阅 [环境准备](/guide/prerequisites)。CLI 的可执行文件路径可在 [常规设置 → CLI 命令](/settings/general#cli-命令) 中自定义。

---

## 多 CLI 架构

自 v0.7.0 起，应用支持多套 CLI 工具链，可在同一工作区中混合使用；自 v0.9.0 起新增第三个家族 **OpenCode CLI**，自 **v0.10.0** 起新增第四个家族 **Kimi Code**：
...
| Kimi Code | Kimi 图标 | 月之暗面 Kimi K 系列模型，支持 Kimi 账号登录或复用应用已配置的第三方供应商（v0.10.0 起） |

在任务的选择器栏中点击 CLI 图标或名称，即可在各 CLI 之间切换。四个家族的模型、思考深度、权限等配置各记各的、互不串台。
...
选择本任务使用的工具链：**Claude CLI**、**Codex CLI**、**OpenCode CLI** 或 **Kimi Code**。选择后，下方的模型、思考深度、执行授权选项会随之切换为对应工具链的选项。
...
**Kimi Code 模型**（v0.10.0 起）分两组：

| 分组 | 内容 |
|------|------|
| **Kimi 账户** | 原生档位 **K2.7 Coding**、**K2.7 Coding Highspeed**、**K3**，需先完成 [Kimi 账号登录](/settings/providers#oauth-供应商登录)；档位清单数据驱动，会话运行中打开面板会从 CLI 实时拉取最新档位 |
| **供应商模型** | 复用应用已配置的[模型供应商](/settings/providers)——Anthropic 或 OpenAI 接口地址任一已填写的启用供应商，其模型全部列出，经环境变量注入 Kimi Code，无需重复填写 API Key |

模型可在会话中途切换、即时生效；上下文用量指示器按所选模型取真实窗口（K3 为 1M token，K2.7 系为 256K）。
...
**Kimi Code 思考深度**（v0.10.0 起）：选择**供应商模型**时提供 default / off / minimal / low / medium / high / max 七档（首档 default 表示不指定、使用该模型的内置默认，同时保护不支持推理的模型），切换后自动以新配置重启子进程、真实生效；选择 **Kimi 账户**原生模型时当前仅有单一档位，选择器自动隐藏，待 Kimi 侧开放多档后恢复显示。

**出厂默认：**
- Claude CLI：max
- Codex CLI：medium
- OpenCode CLI：default（不指定，跟随模型内置默认）
- Kimi Code：default（供应商模型时；原生模型无需选择）
...
**Kimi Code 权限模式**（v0.10.0 起，界面显示原文）：

| 模式（界面原文） | 说明 |
|------|------|
| **default** | 每次执行命令、改文件前弹出审批卡片，等你确认 |
| **plan** | 只读规划模式，先产出计划再经你审批 |
| **auto** | 自动放行安全操作，高风险操作仍会询问（默认档） |
| **yolo** | 全部放行，不再逐条询问 |
...
::: warning 出厂默认为完全自主
Claude CLI 的出厂默认即为"完全自主"——该模式与 Codex 的"完全访问"、OpenCode 的 `auto`、Kimi 的 `yolo` 都会跳过逐条确认（或大幅减少询问），CLI 可在无人值守情况下修改、删除文件或执行系统命令。若你希望逐步确认每个操作，请改为 `default` / `plan`（Claude / Kimi）、"只读"（Codex）或 `ask`（OpenCode）。建议仅在受控环境（沙盒、CI）或你完全了解任务内容时使用完全自主模式。
:::
...
Kimi Code 任务的对话体验与其他家族对齐（实时流式回复、思考与正文分流、工具卡、审批卡、子智能体子卡片、`/` 命令与 `@` 文件、图片输入、历史还原、[深度搜索](/features/deep-search)等），但受其协议限制，v0.10.0 暂不支持以下两项，相关入口会自动隐藏：

- **分叉到新会话**（[会话分叉](/features/conversation#会话分叉)）
- **编辑历史消息重跑**（[编辑历史消息](/features/conversation#编辑历史消息重跑)）
