...
LoongCode 应用本身可从 GitHub Releases 免费下载，源码仓库公开发布（仓库根目录目前未附带开源许可证文件，如有疑问请以官方发布说明为准）。

使用 AI 功能需要本机安装 Claude CLI 或 Codex CLI，AI 服务的费用（如 API 调用计费、订阅计划等）由各 CLI 对应的供应商决定，与 LoongCode 本身无关，请参阅你所使用的 CLI 文档了解具体计费规则。
...
### Claude 和 Codex 任务可以同时用吗？

可以。每个任务在创建时可独立选择使用 Claude CLI 或 Codex CLI，两者之间互不影响，可同时运行多个不同 CLI 的任务。此能力自 v0.7.0 起支持。
...
数据库文件为目录下的 `loongcode.db`。你可以在**设置 → 常规 → 数据目录**中查看当前路径或迁移到自定义位置。
...
目前仅 **Windows** 支持应用内自动更新——当有新版本时，右上角会显示更新徽标，点击即可升级。

**macOS** 暂不支持自动更新，需要手动从 GitHub Releases 下载最新的 `.dmg` 文件覆盖安装。
