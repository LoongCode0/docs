...
| 网络 | 下载安装包需联网访问 LoongCode 官网；使用时需联网访问 AI 服务 |
| 前置依赖 | 需要安装 Claude Code CLI、Codex CLI、OpenCode CLI 或 Kimi Code CLI 中的至少一个（见[前置依赖](/guide/prerequisites)） |
...
1. 前往 [LoongCode 官网](https://loongcode.cc/zh/#download) 下载最新版安装包。
...
::: warning 为什么会出现这个提示？
LoongCode 暂未购买操作系统级代码签名证书，因此被 Windows 标记为「未知发布者」——这是 SmartScreen 对所有未签名软件的标准提示，不代表软件有问题。请确认你的下载链接直接来自官方网站（loongcode.cc），不经任何第三方转发，即可放心安装。
:::

## macOS 安装

1. 前往 [LoongCode 官网](https://loongcode.cc/zh/#download) 下载最新的 `.dmg` 文件（Intel 和 Apple Silicon 共用同一个通用包）。
2. 双击 `.dmg` 文件，将弹出安装窗口。
...
由于 LoongCode 暂未购买 Apple 代码签名证书，首次启动时 macOS 可能提示「无法验证开发者」或「已损坏，无法打开」。这是 macOS 对所有未签名应用的标准提示，不代表软件有问题——只要安装包直接下载自官方网站（loongcode.cc），即可放心按以下方式打开。
